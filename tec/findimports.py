...
def recursive_file_walk_iterator_with_filepath_filter(
    root_folder, filt: str | Callable = None, return_full_path=True
):
...
def count_imports(
    ...
    :param pathfilt: Path filter function (applies to paths and returns True iff the path should be included)
...
class ImportFinder(ast.NodeVisitor):
    ...
    def processImport(self, name, imported_as, full_name, level, node):
        lineno = adjust_lineno(self.filename, self.lineno_offset + node.lineno, name)
...
class ImportFinderAndNameTracker(ImportFinder):
    ...
    def processImport(self, name, imported_as, full_name, level, node):
        ...
        if imported_as != "*":
            ...
            if self.warn_about_duplicates and self.scope.haveImport(imported_as):
                ...
                if "#" not in line:
                    print(
                        "{filename}:{lineno}: {name} imported again".format(
                            filename=self.filename, lineno=lineno, name=imported_as
                        ),
                        file=sys.stderr,
                    )
                    if self.verbose:
                        print(
                            ...
                            " import)".format(filename=self.filename, lineno=where),
                            file=sys.stderr,
                        )
            else:
...
class ModuleGraph:
    ...
    def parseFile(self, filename):
        ...
        if self.trackUnusedNames:
            module.imported_names, module.unused_names = find_imports_and_track_names(
    ...
    def printImports(self):
        ...
        for module in self.listModules():
            ...
            if self.external_dependencies:
                ...
                imports = [
                    modname for modname in module.imports if modname in self.modules
                ]
            imports.sort()
            print("  %s" % "\n  ".join(imports))
    ...
    def printDot(self):
        ...
        for n, module in enumerate(self.listModules()):
            ...
            print('  {}[label="{}"];'.format(module._dot_name, quote(module.label)))
        ...
        for modname, module in sorted(self.modules.items()):
            for other in sorted(module.imports):
                if other in nameDict:
                    print(
                        "  {} -> {};".format(nameDict[module.modname], nameDict[other])
                    )
        print("}")
...
def main(argv=None):
    ...
    parser = optparse.OptionParser(
        "%prog [options] [filename|dirname ...]", prog=progname, description=description
    )
    parser.add_option(
        "-i",
        "--imports",
